...
The container will load the new model from the mounted `ml/saved_model` directory on startup.
...
| `MAX_BATCH_SIZE`       | Maximum customers allowed in one batch request          |
| `API_TITLE`            | Title shown in Swagger UI and OpenAPI schema            |
...
```text
churn_predictions_total{predicted_class="Churn"}
churn_predictions_total{predicted_class="No Churn"}
```
...
A basic load test (`load_test.py`) sends concurrent requests to `/api/v1/predict` to verify behavior under load; results and findings are documented in `TESTING.md`.
...
<!-- Fill in once deployed — see Task 20 Part 3 -->
**URL:** `[your deployed URL here, e.g. https://your-app-name.onrender.com]`
